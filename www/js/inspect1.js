 ...
 //   ])]);
  ...
  page.layout = function(noDraw) { // in the initialization phase, it is not yet time to _draw, and adjust the transform
    ...
    */
    ...
    var topHt = topbarDiv._element.offsetHeight;// was jquery .height()
    ...
    //var actionHt = actionDiv._element.outerHeight();//+(isTopNote?25:0);
    var actionHt = actionDiv._element.offsetHeight;//+(isTopNote?25:0);
  ...
  page.setFlatMode = function(vl) {
    flatMode = vl;
    tree.enabled = !vl;
    obDivTitle._element.innerHTML = flatMode?"Selected Item":"Workspace";
    return;//putback
    if (!vl) {
...
// returns "ok", or an error message
  ...
  function prototypeSource(x) {
    ...
    return om.pathExceptLast(p._source);// without the /source.js
  }
