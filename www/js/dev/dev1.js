...
//start extract
  ...
  var modeTab = ui.modeTab = dom.Tab.mk(['Objects','Code','Requires'],'Objects');
  modeTab.build();
        ...
        /*tree.dataContainer = html.Element.mk('<div id="dataContainer" style="display:none;position:absolute;background-color:white;border:solid thin black;width:100%"/>').addChildren([
          ...
          tree.componentContainer = html.Element.mk('<div id="components" style="display:none;background-color:white;display:none"/>').addChildren([
            tree.componentsDiv = html.Element.mk('<div id="componentDiv" style="position:absolute;background-color:white;width:100%;\
                                                 height:100%;border:solid thin black;overflow:auto;vertical-align:top;margin:0px;padding:'+treePadding+'px"/>').addChildren([
              ui.componentMessage = componentMsg = html.Element.mk('<div style="padding-left:msgPadding"/>')
            ]),
          ]),
  ...
  ui.layout = function(noDraw) { // in the initialization phase, it is not yet time to __draw, and adjust the transform
    ...
    tree.obDiv.$css({width:(svgwd+"px"),height:(treeHt+"px"),top:"0px",left:"0px"});
...
pt.selectCallbacks.push(ui.setInstance); 
  ...
  */
  ...
  ui.messageCallbacks.newItemFromChooserStage2 = function (rs) {
    ...
    var ins = ui.useMinified?"/dev":"/devd";
...
/* returns "ok", or an error message
  ...
  */
  ...
  ui.initFsel = function () {
    fsel.options = ["New Build...","Open...","Fork...","Delete"];
    fsel.optionIds = ["new","open","saveAsBuild","delete"];
    var el = fsel.build();
    mpg.addChild(el);
    el.$hide();
  }
...
var aaa = ((ui.itemOwner)?'':'Since you don\'t own this item, the result of the build is not saved either.')
 ...
 var helpHtml = function () {
  ...
  '<p><span>See this </span><a href="/dev?item=/sys/repo0/example/BarChart2&amp;intro=1">introductory example</a><span> for basic instructions, which appear at the bottom of the introductory example page.</span></p>';
 }
...
ui.shareBut.$click(function () {   
  ...
  ui.messageCallbacks.openItem = function (spath) {
    var devD = ui.useMinified?"/dev":"/devd";
    var url = devD + "?item="+spath;
    location.href = url;
  }
