...
ui.messageCallbacks.saveBuildDone = function (rs) {
 ...
 // function addComponentEl(nm,repo,path) {
  function addComponentEl(xit) {
    ...
    if (!ui.repoIsFromPJ(repo)) {
      pj.error("Repos from elsewhere than pj: not yet");
    }
    ...
    var vinp = html.Element.mk('<input type="input" value="'+id+'" style="font:'+tree.inputFont+
    ...
    if (editable) {
      vinp.addEventListener('blur',function () {
        var newid = vinp.$prop('value');
        if (pj.checkName(newid)) {
          if (componentByName(newid)) {
  ...
  */
  ...
  ui.addComponent = function (iid,path,cb) {
    if (cb) cb();
    var sp = path.split("/");
    var h = sp[0];
    var r = sp[1];
    ...
    //}
    var ids = {};
    cmps.forEach(function (cm) {ids[cm.id] = 1}); 
    var anm = pj.autoname(ids,id);
    var xit = pj.XItem.mk(anm,rr,p);
    cmps.push(xit);//pj.lift({name:nm,repo:rr,path:p}));
    addComponentEl(xit);
  ...
  function toEditMode() {
    ...
    if (firstEdit) {
      ...
      showSource(pj.pathReplaceLast(srcUrl,"source.js"));//unpackedUrl.url+"/source.js");
      firstEdit = false;
    }
  }
