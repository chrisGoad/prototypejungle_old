(function (__pj__) {
  ...
  */
  ...
  function adopt(node,nm,c) {
    if (om.isNode(c)) {
      ...
      om.error('Only Nodes and atomic values can be set as _children in <Node>.set("'+nm+'",<val>)');
    } 
  }
  ...
  // each of its Node _children has a __setIndex__, which was the value of __setCount__ when it was set
 ...
 // __pj__.set("anon",om.DNode.mk()); // where code from the anonymous items lives
  ...
  var properProperty = function (nd,k,knownOwn) {
    ...
    var tp = typeof v;
    if ((tp === "object" ) && v) {
      return om.isNode(v) && (v.__parent__ === nd)  && (v.__name__ === k);
    } else {
      return true;
    }
  };
  ...
  om.DNode.iterAtomicNonstdProperties = function (fn,allowFunctions,isoFar) {
    ...
    pr.iterAtomicNonstdProperties(fn,allowFunctions,soFar);
  }
  ...
  */
  ...
  om.DNode._addOverride = function (dst,root) {
  ...
  om.DNode._addOverridesForInsert = function(root,ovrRoot) {
