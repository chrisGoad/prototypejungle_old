 ...
 * or paths of the form componentName/.../  or /<internalpath> such as /pj/DNode or /svg/g.  ./path is used for references
...
pj.DNode.__isProtoChild = function () {
  ...
  if (!pj.DNode.isPrototypeOf(protoParent)) return false;
...
pj.externalizeDNode = function (node,rootin) {
  ...
  pj.mapOwnProperties(node,function (child,prop) {
    ...
    if (!pj.treeProperty(node,prop,1)) { //1 means includeLeaves
      ...
      childReference = pj.refPath(child,root);
...
pj.stringify = function (node,repo) {
  ...
  rs = 'prototypeJungle.pj.assertItemLoaded('+jsonX+');\n';
