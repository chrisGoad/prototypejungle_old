...
(function (pj) {
  ...
  pj.defineSpread = function (groupConstructor) {
    ...
    var categoryCounts = function categoryCounts(dt, startIndex) {
      var dln = dt.length;
      var rs = {};
      var perEl = function perEl(el) {
    ...
    var buildInstanceSupply = function buildInstanceSupply(marks, ip, dt, byCategory) {
      // for dataless spreads, dt will be the count
    ...
    pj.Spread.generateMark = function (instanceSupply, n, element, byCategory) {
      ...
      if ((typeof rs === 'undefined' ? 'undefined' : _typeof(rs)) === 'object') rs.__show(); //ie not '__modified'
      return rs;
    };
    /*
    ...
    pj.Spread.mk = function (mp) {
      // multiPrototype is the default
    ...
    var mkTheMarks = function mkTheMarks(arrayData) {
      var rs;
      if (arrayData) {
        rs = pj.Array.mk();
      } else {
    ...
    var modificationName = function modificationName(n) {
    ...
    pj.Spread.replacePrototype = function (newProto) {
      ...
      if (categories) {
        ...
        perCategory = function perCategory(category) {
  ...
  geom.normalizeAngle = function (a) {
  ...
  geom.movetoInGlobalCoords = function (nd, x, y) {
  ...
  geom.Rectangle.scaleCentered = function (sc) {
  ...
  geom.Rectangle.plus = function (p) {
    // __translate
  ...
  geom.degreesToRadians = function (n) {
    return Math.PI * (n / 180);
  };

  geom.radiansToDegrees = function (n) {
  ...
  */
  ...
  var addEventListeners = function addEventListeners(el) {
    var cv = el;
  ...
  pj.Object.__iterDomTree = function (fn) {
    ...
    ownprops.forEach(function (k) {
      var ch;
      if (pj.treeProperty(thisHere, k, true, true)) {
  ...
  pj.setChildHooks.push(function (node, nm, c) {
    ...
    if (pj.__isDomEL(node)) {
      ...
      if (nm === "transform" && geom.Transform.isPrototypeOf(c)) {
  ...
  dom.Element.__rootElement = function () {
  ...
  var addListenFors = function addListenFors(nd) {
  ...
  svg.Element.mk = function () {
    return Object.create(svg.Element);
  };
  ...
  svg.showRectangle = function (bnds) {
    ...
    pj.root.set(nm, svg.Element.mk('<rect x="' + crn.x + '" y="' + crn.y + '" width="' + ext.x + '" height="' + ext.y + '" stroke="red" stroke-width="2" fill="transparent"/>'));
  };
  ...
  tag.text.update = function () {
    var d = this.__data;
    var tp = typeof d === 'undefined' ? 'undefined' : _typeof(d);
    if (tp === "number") {
      this.setText(d + "");
    } else if (tp === "string") {
      this.setText(d);
  ...
  */
  var isVisible = function isVisible(inh) {
    return svg.Element.isPrototypeOf(inh) && inh.__visible();
    //code
  };

  pj.Object.__updateVisibleInheritors = function () {
    pj.updateInheritors(this, function (x) {
      x.__update();
    }, isVisible);
  };
  ...
  svg.updateVisibleParts = function (node) {
    pj.updateParts(node, function (part) {
      return svg.Element.isPrototypeOf(part) && part.__visible();
      //code
    });
  };
  ...
  var stashPreSave = function stashPreSave(itm, needRestore) {
  ...
  pj.beforeStringify.push(function (itm) {
    stashPreSave(itm, 1);
  });

  var restoreAfterSave = function restoreAfterSave(itm) {
