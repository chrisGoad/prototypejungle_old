...
(function (__pj__) {
  ...
  // and  __currentXdata holds that value, or it might be set by update, in  which __computedData__ is set.
 ...
 // When an item is loaded with a url of the form ?item=...&data=... the data __get arg sets (or overrides) dataSource.
  ...
  dataOps.OrdinalScale.set("coverage",10); // the number of __values
  ...
  om.LNode.dataDescriptor = function () {
    return this.__ancestorWithProperty("fields");
  }
  ...
  dataOps.Series.mk = function (dt) {
    ...
    rs.__xferLifted(dt,["categories","categoryCaptions"]);
    rs.set("elements",nels);
    rs.__setProperties(dt,["title"]);
    return rs;
  }
  ...
  dataOps.Series.groupByDomain  = function () {
    ...
    // also record the order in which domain __values appear
  ...
  dataOps.Series.map = function (fn) {
    ...
    rs.__setProperties(this,["caption"]);
    return rs;
  }
  ...
  om.stashedData = {};
  om.nodeMethod("__stashData1",function (sd) {
    if (this.__outsideData) {
  ...
  om.DNode.__stashData = function () {
    om.stashedData = {};
    this.__stashData1(om.stashedData);
  }
  
  om.nodeMethod("__restoreData1",function (sd) {
    if (!sd) return;
    var d = sd.__data__;
    if (d) {
      this.data = d;
    ...
    this.__iterTreeItems(function (nd) {
      var nm = nd.__name;
      if (nm!=="data") {
         nd.__restoreData1(sd[nm]);
      }
    },true);
  });
  
  om.DNode.__restoreData = function () {
    this.__restoreData1(om.stashedData);
  }

 

  om.nodeMethod("__dataTransform",function () {
    var anc = this.__ancestorWithProperty("__transform");
...
// where only the domain is transformed, eg 1d bubble charts
  om.nodeMethod("__dataTransform1d",function () {
    var anc = this.__ancestorWithProperty("__transform1d");
    if (dataOps.Data.isPrototypeOf(anc)) {
  ...
  om.processIncomingData = function (xdt) {
    if (xdt) {
      om.root.__currentXdata = xdt;
    } else {
      xdt = om.root.__currentXdata;
    }
    om.root.__internalizeData(xdt);
  }
  ...
  om.performUpdate = function (noCatch,errEl) {
    ...
    if (om.root.update) {
      ...
      om.root.__installOverrides(om.overrides);
  ...
  om.processIncomingItem = function (rs) {
    ...
    if (unbuilt) {
      ...
      if (inst) {
        ...
        __pj__.set("ws",frs);
        frs.__source = pj.page.unpackedUrl.url;
