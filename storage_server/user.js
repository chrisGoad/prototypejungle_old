...
exports.setPassword = function (user,password,cb) {
  var hashed = exports.hashPassword(password);
  var uname = user.name;
  var tm = user.create_time;
  if (!tm) {
    tm = Math.floor(Date.now()/1000)+'';
  ...
  dyndb.putItem(
    {TableName:'pj_user',Item:{'name':{'S':uname},'handle':{'S':handle},'create_time':{'N':tm},'password':{'S':hashed}}},function (e,d) {
      util.log("user","putHandle in pj_user ",uname,handle,e,d);
      });
}
